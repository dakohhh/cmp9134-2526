...
import { getAllUsersV1, updateUserRoleV1, type UsersDataV1, type UserEntryV1 } from '../api/v1'
...
export function Users() {
  ...
  const [data, setData] = useState<UsersDataV1 | null>(null)
  ...
  const fetchUsers = useCallback(async (p: number) => {
    ...
    setError(null)
    try {
      const result = await getAllUsersV1(p, PER_PAGE)
      setData(result)
    } catch (e) {
  ...
  useEffect(() => {
    ...
    fetchUsers(page)
  }, [isCommander, navigate, page, fetchUsers])

  const handleRoleToggle = async (entry: UserEntryV1) => {
    const newRole = entry.role.toUpperCase() === 'VIEWER' ? 'COMMANDER' : 'VIEWER'
    setUpdatingId(entry.id)
    try {
      await updateUserRoleV1(entry.id, newRole)
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-white/10 bg-white/[0.04]">
                      ...
                      <th className="px-4 py-3 text-[10px] uppercase tracking-wider text-muted font-semibold">Email</th>
                      <th className="px-4 py-3 text-[10px] uppercase tracking-wider text-muted font-semibold">Role</th>
                      <th className="px-4 py-3 text-[10px] uppercase tracking-wider text-muted font-semibold">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {results.map((entry: UserEntryV1) => (
                      <tr key={entry.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                        ...
                        <td className="px-4 py-3 text-muted text-xs">{entry.email}</td>
                        <td className="px-4 py-3">
                          <span className={`text-[10px] uppercase tracking-widest px-2 py-1 rounded-full font-semibold border ${
                            entry.role.toUpperCase() === 'COMMANDER'
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className="text-xs px-3 py-1.5 rounded-lg border border-white/10 bg-white/[0.04] text-muted hover:bg-white/[0.08] hover:text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                          >
                            {updatingId === entry.id
                              ? 'Updating…'
                              : entry.role.toUpperCase() === 'VIEWER'
                              ? 'Promote'
                              : 'Demote'}
                          </button>
                        </td>
