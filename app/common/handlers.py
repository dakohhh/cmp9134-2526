...
from app.token.exception import TokenError, TokenBackendError
...
def configure_error_middleware(app: FastAPI) -> None:
    ...
    async def token_error_handler(request: Request, exc: TokenError) -> JSONResponse:
        ...
        return JSONResponse(content={"message": str(exc), "data": None}, status_code=status.HTTP_401_UNAUTHORIZED)

    @app.exception_handler(TokenBackendError)
    async def token_backend_error_handler(request: Request, exc: TokenBackendError) -> JSONResponse:
        # Use Sentry to capture exception
        return JSONResponse(content={"message": str(exc), "data": "ACCESS_TOKEN_EXPIRED"}, status_code=status.HTTP_401_UNAUTHORIZED)
